...
export default function Employees() {
  ...
  const {  valores, valoreseditar, errores,submitForm, actualizarData,handleDelete, handleSubmit,handleChange, handleBlur, guardarValores,guardarEditarValores, guardarActualizarData } = useValidacion(STATE_INICIAL, validarCrearDepartamento, crearEmployee );
  const {employees} = useEmployees(submitForm, actualizarData, guardarActualizarData);
  //para el modal
  ...
  useEffect(() => {
    ...
    editando();
    guardarDatosTabla({data: employees, columns: COLUMNS});
  ...
  const body = (
    <div style={modalStyle} className={classes.paper}>
      <Titulo1>Create New Employee</Titulo1>
      <Formulario
   ...
   async function crearEmployee(){
    ...
    if(editandoValores===true) //si estamos editando un departamento
      {
         //actualizar la bd
         firebase.db.collection('employees').doc(valores.id).update({
          code, 
          name
        });
  ...
  const deleteRegister = async( e, data_borrar) => {
   ...
   try{
       await firebase.db.collection('employees').doc(data_borrar.id).delete();
       handleDelete();
    }catch(error){
      console.log(error);
    }
