...
export default function Currencies() {
  ...
  const {  valores,valoreseditar, errores,actualizarData,submitForm, handleSubmit,handleChange, handleBlur, handleDelete, guardarValores,guardarEditarValores, guardarActualizarData } = useValidacion(STATE_INICIAL, validarCrearDepartamento, crearCurrency );
  const {currencies} = useCurrencies(submitForm, actualizarData, guardarActualizarData);
  ...
  useEffect(() => {
    ...
    editando();
    guardarDatosTabla({data: currencies, columns: COLUMNS});
  ...
  const body = (
    <div style={modalStyle} className={classes.paper}>
      <Titulo1>Create New Currency</Titulo1>
      <Formulario
   ...
   async function crearCurrency(){
 
    //si el usuario no esta autenticado, llevar al login
    if(!usuario){
        return router.push('/login');
    }
    if(editandoValores===true) //si estamos editando una moneda
      {
         //actualizar la bd
         firebase.db.collection('currencies').doc(valores.id).update({
          code, 
          name
        });
        guardarEditandoValores(false);
          ...
          //crear el objeto de nuevo account
            ...
            firebase.db.collection('currencies').add(currency);
      }
     //se limpia la pantalla y todas las variables
     guardarValores(STATE_INICIAL);
     guardarEditarValores(null);
  ...
  const deleteRegister = async( e, data_borrar) => {
   ...
   try{
       await firebase.db.collection('currencies').doc(data_borrar.id).delete();
       handleDelete();
    }catch(error){
      console.log(error);
    }
