...
export default function Departments() {
  ...
  const {  valores,valoreseditar, errores,submitForm, actualizarData, handleSubmit,handleChange, handleBlur, handleDelete, guardarValores,guardarEditarValores, guardarActualizarData } = useValidacion(STATE_INICIAL, validarCrearDepartamento, crearDepartamento );
  const {departamentos} = useDepartamentos(submitForm, actualizarData, guardarActualizarData);
  ...
  useEffect(() => {
    ...
    editando();
    guardarDatosTabla({data: departamentos, columns: COLUMNS});
  ...
  const body = (
    <div style={modalStyle} className={classes.paper}>
      <Titulo1>Create New Department</Titulo1>
      <Formulario
   ...
   async function crearDepartamento()
   {
    ...
    if(editandoValores===true) //si estamos editando un departamento
      {
         //actualizar la bd
         firebase.db.collection('departamentos').doc(valores.id).update({
          code, 
          name
        });
        guardarEditandoValores(false);
          ...
          //crear el objeto de nuevo account
            ...
            firebase.db.collection('departamentos').add(departamento);
      }
     //se limpia la pantalla y todas las variables
     handleClose();
  }
  ...
  const deleteRegister = async( e, data_borrar) => {
   ...
   try{
       await firebase.db.collection('departamentos').doc(data_borrar.id).delete();
       handleDelete();
    }catch(error){
      console.log(error);
    }
